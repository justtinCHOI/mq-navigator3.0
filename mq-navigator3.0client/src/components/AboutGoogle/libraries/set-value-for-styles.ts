...
export function setValueForStyles(
  ...
  if (prevStyles == null) {
    ...
    for (const styleName in styles) {
      ...
      setValueForStyle(elementStyle, styleName, styles[styleName as keyof CSSProperties]);
    }
  ...
  for (const styleName in prevStyles) {
    if (
      Object.prototype.hasOwnProperty.call(prevStyles, styleName) &&
      (styles == null || !Object.prototype.hasOwnProperty.call(styles, styleName))
    ) {
      // Clear style
  ...
  for (const styleName in styles) {
    ...
    if (
      Object.prototype.hasOwnProperty.call(styles, styleName) &&
...
function setValueForStyle(elementStyle: CSSStyleDeclaration, styleName: string, value: unknown) {
  const isCustomProperty = styleName.indexOf('--') === 0;
