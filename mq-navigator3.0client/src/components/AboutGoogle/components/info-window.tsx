...
export const InfoWindow = (props: PropsWithChildren<InfoWindowProps>) => {
  ...
  // ## open info window when content and GoogleMap are available
  const map = useMap();
  useEffect(() => {
    ...
    return () => {
      ...
      // anchor was removed from the GoogleMap before infoWindow.close() is called but the it gets
      // added back to the GoogleMap after that.
  ...
  return (
    <>
      {contentContainerRef.current && createPortal(children, contentContainerRef.current)}

      {headerContainerRef.current !== null && createPortal(headerContent, headerContainerRef.current)}
    </>
  );
};
