...
function useMarker(props: MarkerProps) {
  ...
  const { onClick, onDrag, onDragStart, onDragEnd, onMouseOver, onMouseOut, ...markerOptions } = props;

  const { position, draggable } = markerOptions;

  // create marker instance and add to the GoogleMap once the GoogleMap is available
  useEffect(() => {
    if (!map) {
...
/**
 * Component to render a marker on a GoogleMap
 */
