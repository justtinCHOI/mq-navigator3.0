...
import { useMapsLibrary } from '@components/AboutGoogle';
...
export const AdvancedMarkerContext = React.createContext<AdvancedMarkerContextValue | null>(null);
...
const MarkerContent = ({ children, styles, className, anchorPoint }: MarkerContentProps) => {
  const [xTranslation, yTranslation] = anchorPoint ?? AdvancedMarkerAnchorPoint['BOTTOM'];
...
function useAdvancedMarker(props: AdvancedMarkerProps) {
  ...
  // create an AdvancedMarkerElement instance and add it to the GoogleMap once available
  useEffect(() => {
  ...
  usePropBinding(marker, 'collisionBehavior', collisionBehavior as google.maps.CollisionBehavior);
...
export const AdvancedMarker = forwardRef((props: AdvancedMarkerProps, ref: Ref<AdvancedMarkerRef>) => {
  const { children, style, className, anchorPoint } = props;
  const [marker, contentContainer] = useAdvancedMarker(props);

  const advancedMarkerContextValue: AdvancedMarkerContextValue | null = useMemo(
    () => (marker ? { marker } : null),
    [marker],
  );
  ...
  return (
    <AdvancedMarkerContext.Provider value={advancedMarkerContextValue}>
      {createPortal(
        <MarkerContent anchorPoint={anchorPoint} styles={style} className={className}>
          {children}
        </MarkerContent>,
        contentContainer,
      )}
