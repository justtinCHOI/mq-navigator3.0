import React, { PropsWithChildren, ReactElement, useCallback, useEffect, useMemo, useReducer, useState } from 'react';
...
export const APIProviderContext = React.createContext<APIProviderContextValue | null>(null);
...
/**
 * local hook to set up the GoogleMap-instance management context.
 */
function useMapInstances() {
  const [mapInstances, setMapInstances] = useState<Record<string, google.maps.Map>>({});
...
function useGoogleMapsApiLoader(props: APIProviderProps) {
  const { onLoad, onError, apiKey, version, libraries = [], ...otherApiParams } = props;

  const [status, setStatus] = useState<APILoadingStatus>(GoogleMapsApiLoader.loadingStatus);
  const [loadedLibraries, addLoadedLibrary] = useReducer(
    (loadedLibraries: LoadedLibraries, action: { name: keyof LoadedLibraries; value: LoadedLibraries[string] }) => {
      return loadedLibraries[action.name] ? loadedLibraries : { ...loadedLibraries, [action.name]: action.value };
    },
    {},
  );
 ...
 */
export const APIProvider = (props: PropsWithChildren<APIProviderProps>): ReactElement | null => {
  ...
  return <APIProviderContext.Provider value={contextValue}>{children}</APIProviderContext.Provider>;
};
