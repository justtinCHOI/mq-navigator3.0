...
/**
 * Handlers for all events that could be emitted by GoogleMap-instances.
 */
export type MapEventProps = Partial<{
  // GoogleMap view state events
 ...
 * @internal
 */
export function useMapEvents(map: google.maps.Map | null, props: MapEventProps) {
...
/**
 * Create the wrapped GoogleMap-events used for the event-props.
 * @param type the event type as it is specified to the maps api
 * @param map the GoogleMap instance the event originates from
...
function createMapEvent(
  ...
  if (cameraEventTypes.includes(type)) {
    ...
    if (!center || !bounds || !Number.isFinite(zoom)) {
      console.warn(
        '[createEvent] at least one of the values from the GoogleMap ' +
