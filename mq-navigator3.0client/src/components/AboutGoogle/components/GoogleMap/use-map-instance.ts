...
import { useApiIsLoaded } from '@components/AboutGoogle';
import { CameraState, CameraStateRef, useTrackedCameraStateRef } from './use-tracked-camera-state-ref';

/**
 * Stores a stack of GoogleMap-instances for each mapId. Whenever an
...
/**
 * The main hook takes care of creating GoogleMap-instances and registering them in
 * the api-provider context.
 * @return a tuple of the GoogleMap-instance created (or null) and the callback
 *   ref that will be used to pass the GoogleMap-container into this hook.
 * @internal
 */
export function useMapInstance(
  props: MapProps,
  context: APIProviderContextValue,
): readonly [map: google.maps.Map | null, containerRef: Ref<HTMLDivElement>, cameraStateRef: CameraStateRef] {
  ...
  if (!defaultBounds && (!hasZoom || !hasCenter)) {
    console.warn(
      '<Map> component is missing configuration. ' +
        ...
        '`defaultBounds`. See ' +
        'https://visgl.github.io/react-google-maps/docs/api-reference/components/GoogleMap#required',
    );
  }
  ...
  // create the GoogleMap instance and register it in the context
  useEffect(
    () => {
      ...
      if (reuseMaps && CachedMapStack.has(cacheKey)) {
        ...
        // detaching the element from the DOM lets the GoogleMap fall back to its default
        // size, setting the center will trigger reloading the GoogleMap.
        ...
        map = new google.maps.Map(mapDiv, {
          ...mapOptions,
          ...(renderingType ? { renderingType: renderingType as google.maps.RenderingType } : {}),
      ...
      // prevent GoogleMap not rendering due to missing configuration
      ...
      return () => {
        ...
        // detach the GoogleMap-div from the dom
        mapDiv.remove();
